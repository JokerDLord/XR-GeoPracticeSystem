...
!function (a, b) {
  ...
  function e(b) {
    ...
    this.config.cache_texture && (!x.isInteger(this.config.cache_texture) || this.config.cache_texture < 0) && (this.config.cache_texture = e.DEFAULTS.cache_texture, console.warn("PhotoSphreViewer: invalid valud for cache_texture")), this.config.min_fov = x.bound(this.config.min_fov, 1, 179), this.config.max_fov = x.bound(this.config.max_fov, 1, 179), null === this.config.default_fov ? this.config.default_fov = this.config.max_fov / 2 + this.config.min_fov / 2 : this.config.default_fov = x.bound(this.config.default_fov, this.config.min_fov, this.config.max_fov), this.config.default_long = x.parseAngle(this.config.default_long), this.config.default_lat = x.parseAngle(this.config.default_lat, -Math.PI), this.config.default_lat = x.bound(this.config.default_lat, -x.HalfPI, x.HalfPI), null === this.config.anim_lat ? this.config.anim_lat = this.config.default_lat : (this.config.anim_lat = x.parseAngle(this.config.anim_lat, -Math.PI), this.config.anim_lat = x.bound(this.config.anim_lat, -x.HalfPI, x.HalfPI)), this.config.longitude_range && (this.config.longitude_range = this.config.longitude_range.map(function (a) {
      ...
      pano_data: {full_width: 0, full_height: 0, cropped_width: 0, cropped_height: 0, cropped_x: 0, cropped_y: 0}
  ...
  function i(a) {
    if (f.call(this, a), this.config = this.psv.config.navbar, this.items = [], this.config === !0) this.config = x.clone(e.DEFAULTS.navbar); else if ("string" == typeof this.config) this.config = this.config.split(" "); else if (!Array.isArray(this.config)) {
  ...
  function l(a) {
    f.call(this, a), this.content = null, this.prop = {mouse_x: 0, mouse_y: 0, mousedown: !1, opened: !1}, this.create()
  }

  function m(a) {
    f.call(this, a), this.config = this.psv.config.tooltip, this.prop = {timeout: null}, this.create()
  }
  ...
  c.mixin(e), e.prototype._loadXMP = function (a) {
    ...
    var c = b(), d = new XMLHttpRequest, e = this, f = 0;
    return d.onreadystatechange = function () {
      if (4 === d.readyState) {
        ...
        var a = d.responseText, b = a.indexOf("<x:xmpmeta"), g = a.indexOf("</x:xmpmeta>"), h = a.substring(b, g);
        if (b === -1 || g === -1 || h.indexOf("GPano:") === -1) c.resolve(null); else {
          var i = {
    ...
    return this._loadXMP(c).then(function (d) {
      var f = b(), g = new a.ImageLoader, h = d ? 100 : 0;
    ...
    var d = b(), f = new a.ImageLoader, g = [0, 0, 0, 0, 0, 0], h = [], i = 0;
    ...
    for (var j = function () {
      ...
      this.config.cache_texture && this._putPanoramaCache({panorama: c[b], image: h[b]}), 6 === i && j()
    }, l = function (a, b) {
    ...
    if (this.scene || this._createScene(), this.prop.isCubemap) for (var b = 0; b < 6; b++) this.mesh.material.materials[b].map && this.mesh.material.materials[b].map.dispose(), this.mesh.material.materials[b].map = a[b]; else this.mesh.material.map && this.mesh.material.map.dispose(), this.mesh.material.map = a;
    ...
    for (var b = new a.BoxGeometry(2 * e.SPHERE_RADIUS, 2 * e.SPHERE_RADIUS, 2 * e.SPHERE_RADIUS, e.CUBE_VERTICES, e.CUBE_VERTICES, e.CUBE_VERTICES), c = [], d = 0; d < 6; d++) c.push(new a.MeshBasicMaterial({overdraw: e.SYSTEM.isWebGLSupported && this.config.webgl ? 0 : 1}));
    ...
    var f = new a.MeshBasicMaterial({side: a.BackSide, visible: !1}), g = new a.Mesh(b, f);
    this.scene.add(g)
  }, e.prototype._transition = function (b, c) {
    if (this.prop.isCubemap) throw new v("Transition is not available with cubemap.");
    var d = this, f = new a.SphereGeometry(.9 * e.SPHERE_RADIUS, e.SPHERE_VERTICES, e.SPHERE_VERTICES, (-x.HalfPI)),
    ...
    var a = this, b = -this.config.anim_speed, c = this.config.longitude_range;
    this.config.longitude_range = null, x.animation({
      properties: {speed: {start: this.config.anim_speed, end: 0}},
      duration: 300,
      easing: "inSine",
      onTick: function (b) {
      ...
      return x.animation({
        properties: {speed: {start: 0, end: b}},
        duration: 300,
        easing: "outSine",
        onTick: function (b) {
          a.config.anim_speed = b.speed
    ...
    tooltip: {offset: 5, arrow_size: 7, delay: 100},
    lang: {
    ...
    transition: {duration: 1500, loader: !0},
    loading_img: null,
    loading_txt: "Loading...",
    size: null,
    cache_texture: 5,
    templates: {},
    markers: []
  }, e.TEMPLATES = {markersList: '<div class="psv-markers-list-container">   <h1 class="psv-markers-list-title">{{= it.config.lang.markers }}</h1>   <ul class="psv-markers-list">   {{~ it.markers: marker }}     <li data-psv-marker="{{= marker.id }}" class="psv-markers-list-item {{? marker.className }}{{= marker.className }}{{?}}">       {{? marker.image }}<img class="psv-markers-list-image" src="{{= marker.image }}"/>{{?}}       <p class="psv-markers-list-name">{{? marker.tooltip }}{{= marker.tooltip.content }}{{?? marker.html }}{{= marker.html }}{{??}}{{= marker.id }}{{?}}</p>     </li>   {{~}}   </ul> </div>'}, e.prototype._bindEvents = function () {
    ...
    var b = 0, c = 0, d = 0, f = a.key || e.KEYMAP[a.keyCode || a.which];
    switch (f) {
      case"ArrowUp":
        c = .01;
        break;
    ...
    var b = [{x: parseInt(a.touches[0].clientX), y: parseInt(a.touches[0].clientY)}, {
    ...
    var b = this, c = {x: a.clientX - this.prop.mouse_history[0][1], y: a.clientY - this.prop.mouse_history[0][2]},
      d = Math.sqrt(c.x * c.x + c.y * c.y);
    this.prop.animation_promise = x.animation({
      properties: {
        clientX: {start: a.clientX, end: a.clientX + c.x},
        clientY: {start: a.clientY, end: a.clientY + c.y}
      }, duration: d * e.INERTIA_WINDOW / 100, easing: "outCirc", onTick: function (a) {
        b._move(a, !1)
      }
    }).ensure(function () {
      b.prop.moving = !1
    ...
    var b = this.container.getBoundingClientRect(), c = {
      target: a.target,
      client_x: a.clientX,
      client_y: a.clientY,
      viewer_x: parseInt(a.clientX - b.left),
      viewer_y: parseInt(a.clientY - b.top)
    }, d = this.viewerCoordsToVector3({x: c.viewer_x, y: c.viewer_y});
    if (d) {
      var f = this.vector3ToSphericalCoords(d);
      if (c.longitude = f.longitude, c.latitude = f.latitude, !this.prop.isCubemap) {
        var g = this.sphericalCoordsToTextureCoords({longitude: c.longitude, latitude: c.latitude});
    ...
    if (this.prop.moving) {
      var c = parseInt(a.clientX), d = parseInt(a.clientY);
      this.rotate({
    ...
    if (this.prop.zooming) {
      var b = [{
        ...
        }, {x: parseInt(a.touches[1].clientX), y: parseInt(a.touches[1].clientY)}],
    ...
    return {longitude: this.prop.longitude, latitude: this.prop.latitude}
  }, e.prototype.getZoomLevel = function () {
    return this.prop.zoom_lvl
  }, e.prototype.getSize = function () {
    return {width: this.prop.size.width, height: this.prop.size.height}
    ...
    return this.prop.animation_promise = x.animation({
      properties: {
        ...
        }, latitude: {start: this.prop.latitude, end: a.latitude}
      }, duration: c, easing: "inOutSine", onTick: this.rotate.bind(this)
    ...
    if (a) {
      for (var b = 0, c = this.prop.cache.length; b < c; b++) if (this.prop.cache[b].panorama === a) {
    ...
    return {longitude: b >= Math.PI ? b - Math.PI : b + Math.PI, latitude: x.HalfPI - c}
    ...
    var b = Math.acos(a.y / Math.sqrt(a.x * a.x + a.y * a.y + a.z * a.z)), c = Math.atan2(a.x, a.z);
    return {longitude: c < 0 ? -c : x.TwoPI - c, latitude: x.HalfPI - b}
    ...
    for (var c in this.markers) {
      var d = this.markers[c], e = d.visible;
      if (e && d.isPolygon()) {
    ...
    var b = a.positions3D.length, c = a.positions3D.map(function (a) {
      return {vector: a, visible: a.dot(this.psv.prop.direction) > 0}
    }, this), d = [];
    return c.forEach(function (a, e) {
      if (!a.visible) {
        var f = [0 === e ? c[b - 1] : c[e - 1], e === b - 1 ? c[0] : c[e + 1]];
        f.forEach(function (b) {
          b.visible && d.push({visible: b, invisible: a, index: e})
        })
      }
    ...
    var d = this.psv.prop.direction.clone().normalize(), f = (new a.Vector3).crossVectors(b, c).normalize(),
    ...
    }), a.width = e - c, a.height = f - d, {x: c, y: d}
    ...
    if (!this.psv.prop.moving) {
      ...
      if (a.target && (b = a.target.psvMarker) && b.isPolygon() || a.target && x.hasParent(a.target, this.psv.tooltip.container) && (b = this.hoveringMarker)) {
        ...
        b.tooltip && this.psv.tooltip.showTooltip({
          ...
          box: {width: 2 * this.psv.config.tooltip.arrow_size, height: 2 * this.psv.config.tooltip.arrow_size}
        })
    ...
    f.prototype.create.call(this), this.config.forEach(function (a) {
      if ("object" == typeof a) this.items.push(new p(this, a)); else switch (a) {
        case o.id:
    ...
    var b = parseInt(a.clientX), c = parseInt(a.clientY);
    ...
    .5: "center",
    1: "right"
  },m.topMap = {0: "top", .5: "center", 1: "bottom"},m.prototype.create = function () {
    ...
    var b = this.isTooltipVisible(), c = this.container, d = this.content, e = this.arrow;
    ...
    if (b) for (var g = c.classList.length - 1; g >= 0; g--) {
    ...
    var i = c.getBoundingClientRect(), j = {
  ...
  },o.prototype = Object.create(n.prototype),o.prototype.constructor = o,o.id = "autorotate",o.className = "psv-button psv-button--hover-scale psv-autorotate-button",o.icon = "play.svg",o.iconActive = "play-active.svg",o.prototype.create = function () {
    ...
    if (this.prop.mousedown) {
      var b = parseInt(a) - this.zoom_range.getBoundingClientRect().left, c = b / this.zoom_range.offsetWidth * 100;
      this.psv.zoom(c)
    }
    ...
    switch (this._dynamicSize = !0, this.type) {
      ...
      case"ellipse":
        ...
        break;
      case"path":
        "string" == typeof this._def && (this._def = {d: this._def})
    }
    ...
    this._dynamicSize = !0, this.svgStyle ? Object.getOwnPropertyNames(this.svgStyle).forEach(function (a) {
      ...
      if (a && "object" != typeof a[0]) for (var b = 0; b < a.length; b++) a.splice(b, 2, [a[b], a[b + 1]])
    }), this.polygon_px ? this.polygon_rad = this.polygon_px.map(function (a) {
      var b = this.psv.textureCoordsToSphericalCoords({x: a[0], y: a[1]});
      ...
      return this.psv.sphericalCoordsToVector3({longitude: a[0], latitude: a[1]})
    }, this)
  };
  var x = {};
  return e.Utils = x, x.TwoPI = 2 * Math.PI, x.HalfPI = Math.PI / 2, x.svgNS = "http://www.w3.org/2000/svg", x.checkTHREE = function (b) {
    for (var c = 0, d = arguments.length; c < d; c++) if (!(arguments[c] in a)) return !1;
    return !0
  }, x.isCanvasSupported = function () {
    var a = document.createElement("canvas");
    return !(!a.getContext || !a.getContext("2d"))
  }, x.getWebGLCtx = function () {
    var a = document.createElement("canvas"), b = ["webgl", "experimental-webgl", "moz-webgl", "webkit-3d"], c = null;
    ...
    if (a.classList) void 0 === c ? a.classList.toggle(b) : c && !a.classList.contains(b) ? a.classList.add(b) : c || a.classList.remove(b); else {
      var d = a.getAttribute("class") || "", e = d.indexOf(b) !== -1, f = new RegExp("(?:^|\\s)" + b + "(?:\\s|$)");
    ...
    do if (a === b) return !0; while (a = a.parentNode);
    return !1
  }, x.getClosest = function (a, b) {
    var c = a.matches || a.msMatchesSelector;
    do if (c.bind(a)(b)) return a; while (a = a.parentElement);
    return null
    ...
    for (var b in a) if (b in document) return a[b]
    ...
    return d ? {left: d[1] / 100, top: d[2] / 100} : {top: .5, left: .5}
    ...
    if (a.requestAnimationFrame = a.requestAnimationFrame || a.mozRequestAnimationFrame || a.webkitRequestAnimationFrame || a.msRequestAnimationFrame, a.cancelAnimationFrame = a.cancelAnimationFrame || a.mozCancelAnimationFrame || a.webkitCancelAnimationFrame || a.msCancelAnimationFrame, !a.requestAnimationFrame) {
      ...
      a.requestAnimationFrame = function (a) {
        ...
        var b, e;
        for (b = 0, e = c.length; b < e; b += 1) if (c[b][0] === a) return void c.splice(b, 1);
        for (b = 0, e = d.length; b < e; b += 1) if (d[b][0] === a) return void d.splice(b, 1)
      }
    }
